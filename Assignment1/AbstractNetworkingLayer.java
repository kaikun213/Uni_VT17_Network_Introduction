...
public abstract class AbstractNetworkingLayer {
	...
	byte[] buf;
	int transferRateValue;
	SocketAddress localBindPoint;
	SocketAddress remoteBindPoint;
	double factorRTT = 0.99;	// factor for round trip time
	...
	protected void correctInputs(String[] args){
		...
		if (args.length != 4) {
			...
			System.err.println("Error. Provide a valid buffer size as Integer. The maximum Buffersize is 1.000.000. If you take zero it will not be able to receive any messaes.");
			System.exit(1);
		}
    ...
    protected boolean isValidBufferSize(String s){
    	...
    	/* Therefore max is set to 1 000 000 here to make sure */
    	return (isPositiveInteger(s) && (Integer.parseInt(s)<=1000000));
    }
    
    protected void showNanoSeconds(String s, long nano){
    	System.out.println(s + nano);
    }
